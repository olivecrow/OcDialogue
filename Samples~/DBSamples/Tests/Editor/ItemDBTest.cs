...
namespace MyDB.Test.Editor
{
    public class ItemDBTest
    {
        ...
        {
            foreach (var item in ItemDB.Instance.Items)
            {
                var count = ItemDB.Instance.Items.Count(x => x.itemName == item.itemName);
        ...
        {
            foreach (var item in ItemDB.Instance.Items.Where(x => x.type == ItemType.Generic))
            {
                CommonTest(item);
            }
        }
        ...
        {
            foreach (var item in ItemDB.Instance.Items.Where(x => x.type == ItemType.Armor))
            {
                CommonTest(item);
        ...
        {
            foreach (var item in ItemDB.Instance.Items.Where(x => x.type == ItemType.Weapon))
            {
                CommonTest(item);
        ...
        {
            foreach (var item in ItemDB.Instance.Items.Where(x => x.type == ItemType.Accessory))
            {
                CommonTest(item);
        ...
        {
            foreach (var item in ItemDB.Instance.Items.Where(x => x.type == ItemType.Important))
            {
                CommonTest(item);
            }
        }
        ...
        static bool IsDescriptionValidate(ItemBase item)
        {
            ...
            {
                Debug.LogWarning($"[{item.Address}] Description 이 비어있음");
                return false;
            }
        ...
        static bool IsIconValid(ItemBase item)
        {
        ...
        static bool IsMaxDurabilityValidate(IEquipment equipment)
        {
            ...
            Debug.LogWarning($"[{item.Address}] MaxDurability가 0 이하임.");
            return false;
        }
        ...
        static bool IsWeightValidate(IEquipment equipment)
        {
            ...
            var item = equipment.ItemBase;
            Debug.LogWarning($"[{item.Address}] Weight가 0 이하임.");
            return false;
        }
        ...
        static bool IsEquipmentStackableValid(IEquipment equipment)
        {
            ...
            var item = equipment.ItemBase;
            Debug.LogWarning($"[{item.Address}] 장비 아이템은 isStackable일 수 없음.");
            return false;
        }
